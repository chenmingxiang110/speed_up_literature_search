...
class getRecommendation:
    ...
    def main(self, graph, paperID, num, mode):
        gtb = graphToolBox()
        nodeID = self.transferInput(paperID)
...
class getRecommendation_rndwlk:
    ...
    def main(self, graph, paperID, num):
        nodeID = self.transferInput(paperID)
        if not graph.IsNode(int(nodeID)):
            print "Cannot find the paper"
            return
        setting = raw_input("Using default settings? y/n -> ")
        num_iter = 10000
        forward = 0.45
        backward = 0.2
        if setting is "n":
            num_iter = raw_input("num_iter = ")
            forward = raw_input("forward = ")
            backward = raw_input("backward = ")

        print "Searching..."
        result = randWlk(graph, num_iter, forward, backward).getRecommendation(nodeID, num)
...
class searchByFile:
    ...
    def printInfo(self):
        with open("/home/da_package/downloads/dataset/paper_names.txt", 'r') as f:
            print "Searching..."
            for line in f:
                print ""
                searchByAttributesOne(line).printInfo()


class paperId2PaperName:
    ...
    def seperate(self):
        ...
        with open("../20160205-MicrosoftAcademicGraph/Papers.txt", 'r') as f:
            ...
            for line in f:
                ...
                indica += 1
                thelist = line.split("\t")
                currline = thelist[0]+":"+thelist[2]+"\n"
                line_list.append(currline)

        with open("../20160205-MicrosoftAcademicGraph/Id2Name.txt", 'a+') as fw:
            indica = 0
            for currline in line_list:
                if indica%100000 == 0:
                    print "writing "+repr(indica)+" lines"
                indica += 1
                fw.write(currline)
    ...
    def seperate(self):
        ...
        with open("../20160205-MicrosoftAcademicGraph/Papers.txt", 'r') as f:
            ...
            for line in f:
                ...
                indica += 1
                thelist = line.split("\t")
                currline = thelist[0]+":"+thelist[3]+"\n"
                line_list.append(currline)

        with open("../20160205-MicrosoftAcademicGraph/Id2Year.txt", 'a+') as fw:
            indica = 0
            for currline in line_list:
                if indica%100000 == 0:
                    print "writing "+repr(indica)+" lines"
...
class nodeid2year2cite:
    ...
    def id2year(self):
        ...
        with open("../20160205-MicrosoftAcademicGraph/Id2Year.txt", 'r') as f:
            ...
            for line in f:
                if indica%100000 == 0:
                    print "reading "+repr(indica)+" lines (building the dictionary)"
                indica += 1
                ids = (line.split("\n")[0]).split(":")
                nodeid = ids[0]
                paperid = ids[1]
    ...
    def main(self):
        ...
        print "building id2year dictionary"
        self.id2year_dict = self.id2year()
        ...
        with open("../20160205-MicrosoftAcademicGraph/nodeid_paperid.txt", 'r') as f:
            ...
            for line in f:
                ...
                if not graph.IsNode(int(nodeid)):
                    continue
                citation = gtb.get_num_citation(graph, int(nodeid))
                year = self.id2year_dict[paperid]
                newline = nodeid+","+paperid+","+repr(citation)+","+year+"\n"
                newlines.append(newline)
        with open("../20160205-MicrosoftAcademicGraph/node2id2year2cite.txt", 'a+') as fw:
            indica = 0
            for currline in newlines:
                if indica%100000 == 0:
                    print "writing "+repr(indica)+" lines (writing the file)"
                indica += 1
                fw.write(currline)
